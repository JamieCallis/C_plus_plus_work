...
m_bitmap(m_width, m_height), m_zoomList(m_width, m_height)
	...
	void FractalCreator::calculateRangeTotals()
	{
		...
		for (int numberofiterations = 0; numberofiterations < Mandelbrot::MAX_ITERATIONS; numberofiterations++)
		{
			int pixels = m_histogram[numberofiterations];

			if (numberofiterations >= m_ranges[rangeIndex+1])
			{
				rangeIndex++;
			}
